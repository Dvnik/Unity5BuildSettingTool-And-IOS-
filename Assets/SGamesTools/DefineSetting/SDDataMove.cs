...
public class SDDataMove
{
	...
	public static Texture2D GetSplashScreen(string iScrName)
	{
		Texture2D aResult = null;
		PlayerSettings[] aAllSet = Resources.FindObjectsOfTypeAll<PlayerSettings> ();
		for(int i = 0; i < aAllSet.Length; i++)
		{
			SerializedObject aSO = new SerializedObject(aAllSet[i]);
			aSO.Update();
			aResult = aSO.FindProperty (iScrName).objectReferenceValue as Texture2D;
		}
		return aResult;
	}

	public static string GetSplashScreenPath(string iScrName, ref Texture2D iImage)
	{
		string aResult = string.Empty;
		iImage = GetSplashScreen(iScrName);
		if(iImage != null)
			aResult = AssetDatabase.GetAssetPath(iImage);
	...
	/// </summary>
	public static void SetSplashScreen(string iScrName, string iPath )
	{
		foreach (var player in Resources.FindObjectsOfTypeAll<PlayerSettings> ())
		{
			var so = new SerializedObject (player);
			so.Update ();
			so.FindProperty (iScrName).objectReferenceValue = AssetDatabase.LoadAssetAtPath (iPath, typeof(Texture2D));
			so.ApplyModifiedProperties ();
